...
To compile:
mpicc -Wall -o homework4 homework4_nonblocking.c
...
int main(int argc, char* argv[]) {
    ...
    for (int n = 1; n <= maxGenerations; n++) {
        ...
        MPI_Isend(curBoard[1], boardSize + 2, MPI_INT, rank_above, 0, MPI_COMM_WORLD, &sendReq[0]);
        MPI_Irecv(curBoard[numRows+1], boardSize + 2, MPI_INT, rank_below, 0, MPI_COMM_WORLD, &recvReq[0]);
        // Second set of ghost cell exchanges tagged 1
        MPI_Isend(curBoard[numRows], boardSize + 2, MPI_INT, rank_below, 1, MPI_COMM_WORLD, &sendReq[1]);
        MPI_Irecv(curBoard[0], boardSize + 2, MPI_INT, rank_above, 1, MPI_COMM_WORLD, &recvReq[1]);
